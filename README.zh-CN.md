...
- **GitHub Backup**：把 Vola 可见文件树同步到私有 GitHub 仓库，保留可恢复版本历史。[查看文档](docs/github-backup.zh-CN.md)
- **Hosted 或自托管**：可以直接使用 [vola.ai](https://www.vola.ai)，也可以部署自己的 Hub，使用本地或远端存储。
...
接着在已连接好的客户端里打开一个**新的对话**，直接说一句：`请把这个工作区里有用的 skills、项目上下文和 profile/preferences 导入到 Vola。`
