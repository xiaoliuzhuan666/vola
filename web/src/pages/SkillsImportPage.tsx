...
export default function SkillsImportPage() {
  ...
  const requestError = useMemo(() => {
    if (request.error === 'missing_token' && !hasSessionToken) {
      return tx(
        '请先登录 neuDrive，或使用 neuDrive 生成的带 token 上传链接。',
        'Sign in to neuDrive first, or use a tokenized upload link generated by neuDrive.',
      )
    }
    return ''
  ...
  return (
    <div className="skills-upload-page">
      <div className="skills-upload-card">
        ...
        <div className="sync-login-eyebrow">neuDrive Skills</div>
        <h1 className="login-title">{tx('上传 Skills Zip', 'Upload skills zip')}</h1>
        <p className="login-desc">
          {tx(
            '把完整的 skills zip 上传到 neuDrive。页面会导入 `/skills`，生成 manifest.neudrive.json，并标出脚本、依赖、assets、外部 Claude 引用和环境变量提示。',
            'Upload a complete skills zip into neuDrive. The page imports it into `/skills`, generates manifest.neudrive.json, and flags scripts, dependencies, assets, external Claude references, and environment variable notes.',
          )}
        </p>
            ...
            {manifestWarnings.length > 0 && (
              <div className="alert alert-warn">
                ...
                {manifestWarnings.length > 8 && (
                  <div className="data-record-secondary">
                    {tx(`还有 ${manifestWarnings.length - 8} 条提示，可在 skill 目录的 manifest.neudrive.json 查看。`, `${manifestWarnings.length - 8} more notes are available in manifest.neudrive.json.`)}
                  </div>
                )}
              </div>
            )}
