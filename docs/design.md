...
neuDrive 平台接入现在固定分三层：

1. **MCP 层**
   - 提供 neuDrive 的全量能力面
   - 包括 tree/files、skills、profile/memory、projects、sync token 等现有工具
2. **入口层**
   - 平台原生入口统一命名为 `neudrive`
   - Claude Code：`/neudrive <subcommand>`
   - Codex：`$neudrive <subcommand>` 或 `/skills` 选择 `neudrive`
3. **技能层**
   - 系统级 umbrella skill：`/skills/neudrive/SKILL.md`
   - 平台本地安装物只是入口和步骤说明，不复制 MCP 功能

这意味着：**MCP 是完整能力面，`neudrive` skill / command 是平台内入口面。**
...
这就是 neuDrive 要占据的生态位：**AI 时代的统一信任层**

### 0.2 为什么 IFTTT 没有成为这个东西

IFTTT 验证了跨服务编排的需求，但它停在了极客玩具的位置，从来没有成为"基础设施"。原因有三个，对 neuDrive 的设计有直接启示：

**第一，IFTTT 需要用户主动配置规则。** "如果 Gmail 收到带附件的邮件，就保存到 Dropbox"——这需要用户想清楚自己要什么、用技术语言表达出来、然后维护这些规则。大众用户做不到也不愿意做

neuDrive 的区别：用户不配置规则。用户用自然语言说话（"帮我把这个文件存到 NAS 上"），Agent 通过 Hub 发现有 NAS 可用、知道怎么调用、自动完成。规则是 Agent 实时推断的，不是用户预设的

**第二，IFTTT 没有身份和信任。** 它只做动作触发，不管"你是谁"和"你信任谁"。用户的身份信息、隐私数据、支付凭证，IFTTT 都不碰

neuDrive 的区别：身份和信任是核心。Hub 的第一价值不是"帮你连接服务"，是"任何 Agent 通过 Hub 立刻知道你是谁、你信任它到什么程度、它能做什么"。这是"谷歌登录"级别的能力，不是"Zapier"级别的

**第三，IFTTT 的网络效应不够强。** 我不用 IFTTT，不影响你用 IFTTT。没有跨用户的价值

neuDrive 的区别：当两个用户都有 Hub，他们的 Agent 可以直接通信、共享知识、协同工作。用户越多，每个用户的 Agent 能做的事越多。这是真正的网络效应

### 0.3 neuDrive 要成为什么

一句话：**AI 时代的身份与信任基础设施**

它不是一个 App，不是一个工具，不是一个 SaaS 产品。它是一层协议 + 一个服务，所有 AI Agent 的交互都经过这一层。就像所有网站的登录都可以经过谷歌登录，所有 Agent 的身份验证、上下文获取、权限管理、跨平台通信都经过 neuDrive

长期看，用户看到的应该是这样的东西：

- 在任何 AI 平台的界面上，有一个 **"Connect with neuDrive"** 的按钮（类比"Sign in with Google"）
...
neuDrive 可能的冷启动路径（需要进一步验证，列出几个方向）：

**路径 A：MCP 生态切入**。Claude 已经原生支持 MCP，用户可以直接连接 MCP server。neuDrive 做成一个 MCP server，任何 Claude 用户一行配置就能接入。先在 Claude 生态里获得种子用户，再扩展到其他平台。这是最低成本的起步方式

**路径 B：平台原生接入**。优先利用 Claude Connectors、ChatGPT Apps、Cursor / Windsurf Remote MCP 等官方或稳定接口，让用户在现有 AI 工具里授权 neuDrive，而不是依赖页面结构

**路径 C：社区驱动**。从 AGI Bar 社区和中国 AI 从业者圈子开始，先服务最有影响力的早期用户。这些人每天在多个 AI 平台之间切换，痛点最强烈。他们用起来之后，自然会推荐给自己的圈子

**路径 D：开发者 SDK**。给 Agent 应用开发者提供 SDK："在你的 Agent 应用里加两行代码，你的用户就可以用 neuDrive 登录，自动获得用户的偏好和上下文"。这是"谷歌登录"的复制路径，但需要开发者生态的配合
...
| 平台层 | 抽成/收费：Agent 应用开发者通过 neuDrive SDK 获取用户时的费用 | 开发者生态建成后 |
...
"neuDrive"是工作名，对于一个有基础设施野心的项目来说，名字非常重要。几个考量：
...
1. **对齐认知**：任何新加入的人读完这份文档，应该知道 neuDrive 要解决什么问题、占据什么生态位、怎么解决、以及明确不做什么
...
- **"Connect with neuDrive"什么时候出现？** 什么时候能让第三方 Agent 应用在自己的界面上加一个"Connect with neuDrive"按钮？这是从工具变成基础设施的转折点
