...
本文记录 neuDrive 自部署时的账号开通、存储额度、手机端同步和腾讯云 COS 备份建议。
...
额度统计范围是 neuDrive 文件树里的用户可见数据，包括 Skills、Memory、Projects、Inbox、Vault scope 元数据等；目录本身不计入容量。二进制文件即使存到腾讯 Lighthouse COS，也按文件大小计入用户额度。账号、session、token、连接配置、billing 状态仍在数据库里，不属于这个文件树容量。
...
1. 手机端通过 neuDrive 登录拿到 JWT 或 scoped token。
...
如果用户在手机里的 ChatGPT、Claude 等 App 里使用 neuDrive，通常不需要把数据同步到手机文件系统。那些 Agent 通过云端 MCP / OAuth 访问 neuDrive，数据仍然保存在服务器里。
