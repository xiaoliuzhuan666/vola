...
- 更新检查已接入服务后台调度：服务启动后会立即检查一次，并按周期继续检查团队 Skill 订阅。检测到团队源文件有新版时，会写入成员个人提醒和个人更新通知文件。后续还需要完善前端提醒入口、失败状态展示和管理员视角的处理队列。
...
- 审查记录增加评论线程和多人通过策略。
- 成员登录时看到更明确的“可安装 / 可更新”提醒，并能直接查看来源团队和更新内容。
- 更新检查的调度状态、最近失败原因和处理结果进入管理员页面。
