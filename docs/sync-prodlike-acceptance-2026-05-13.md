# neuDrive prod-like 同步与备份验收记录

日期：2026-05-13

## 环境

- 工作区：`/Users/zhongmoshu/Desktop/work/neuDrive`
- Commit：`2f255a154bcdd5ffc1328ddddb8edc5160e3eb12`
- 分支状态：`main...origin/main`
- 主存储：本地 SQLite
- 服务地址：`http://127.0.0.1:42731`
- SQLite 数据库：`/private/tmp/neudrive-prodlike-20260513/acceptance.db`
- 验收产物目录：`/private/tmp/neudrive-prodlike-20260513`
...
```json
{"ok":true,"data":{"service":"neudrive","status":"ok","storage":"sqlite"}}
```
...
```bash
/private/tmp/neudrive-prodlike-20260513/neu sync export --source /private/tmp/neudrive-prodlike-20260513/fixtures/roundtrip-src --format archive -o /private/tmp/neudrive-prodlike-20260513/roundtrip.ndrvz
/private/tmp/neudrive-prodlike-20260513/neu sync preview --bundle /private/tmp/neudrive-prodlike-20260513/roundtrip.ndrvz --api-base http://127.0.0.1:42731 --token <sync-token>
/private/tmp/neudrive-prodlike-20260513/neu sync push --bundle /private/tmp/neudrive-prodlike-20260513/roundtrip.ndrvz --transport auto --api-base http://127.0.0.1:42731 --token <sync-token>
/private/tmp/neudrive-prodlike-20260513/neu sync pull --format archive -o /private/tmp/neudrive-prodlike-20260513/roundtrip-pulled.ndrvz --api-base http://127.0.0.1:42731 --token <sync-token>
/private/tmp/neudrive-prodlike-20260513/neu sync diff --left /private/tmp/neudrive-prodlike-20260513/roundtrip.ndrvz --right /private/tmp/neudrive-prodlike-20260513/roundtrip-pulled.ndrvz --format json
/private/tmp/neudrive-prodlike-20260513/neu sync resume --bundle /private/tmp/neudrive-prodlike-20260513/resume-large.ndrvz --api-base http://127.0.0.1:42731 --token <sync-token>
go test ./...
cd web && npm run build
make build
```

`make build` 按仓库 Makefile 执行了 `npm ci`、前端构建、`rm -rf internal/web/dist`、复制 `web/dist` 到 `internal/web/dist`，并重新构建 `bin/neudrive` 和 `bin/neu`。
...
| Resume sidecar 文件清理 | yes | `/private/tmp/neudrive-prodlike-20260513/resume-large.ndrvz.session.json` 在 resume 后已不存在。 |
...
| WebDAV 上传代码路径 | yes，本地接收器 | 本地接收器收到 WebDAV `PUT`，带 Basic auth，对象为 `neudrive-export-20260513-031523Z.zip`，大小 34577 bytes。 |
| S3-compatible 上传代码路径 | yes，本地接收器 | 本地接收器收到 S3 path-style `PUT`，带 AWS Signature V4 auth，对象为 `acceptance/neudrive-export-20260513-031627Z.zip`，大小 34610 bytes。 |
...
| Embedded build | yes | `make build` 通过，并刷新 `internal/web/dist`、`bin/neudrive`、`bin/neu`。 |
...
- 没有真实 WebDAV/S3-compatible provider 凭据，真实 provider 上传和删除兼容性未验证。本地接收器只证明 neuDrive 会生成请求并记录 history。
