...
本文记录 neuDrive 当前对 Claude Code、Codex、Cursor、Gemini CLI 的 Skill 同步边界。
...
- neuDrive 会在写入的 Skill 目录内创建 `.neudrive-managed.json`。
- 预览和应用只会更新 neuDrive 管理过的目录；如果目标目录里已经存在同名但没有标记的 Skill，会显示冲突，不会覆盖。
- 清理只删除带 `.neudrive-managed.json` 且已经取消分配的 Skill。

## Codex

- 默认目标目录是 `~/.codex/skills`。
- 行为与 Claude Code 一致：只写分配 Skill，只更新带 `.neudrive-managed.json` 的目录。
- 从 Claude Code 转换到 Codex 的简单 Skill，可以进入 `/skills/<name>-codex`，再通过本地同步写入 Codex Skill 目录。

## Cursor

- 当前状态是“可分配、可导出、暂不自动写入”。
- neuDrive 会记录 Cursor 的 Skill 分配，并在本地同步预览里生成导出项。
- 导出包包含完整 Skill 文件夹：`SKILL.md`、scripts、依赖文件、assets、`external/claude-tools`、`external/claude-plugins`、`manifest.neudrive.json`。
- 如果某个项目使用 `.cursor/rules`，用户需要手动阅读导出包，把适合 Cursor 的内容整理到项目规则里。
- neuDrive 不自动修改 Cursor 全局配置或项目配置。

## Gemini CLI

- 当前状态是“可分配、可导出、暂不自动写入”。
- neuDrive 会记录 Gemini CLI 的 Skill 分配，并在本地同步预览里生成导出项。
- 导出包同样保留完整 Skill 文件夹和 manifest。
- 用户需要手动把可用内容引用或整理到自己项目已有的 `GEMINI.md` 或其他 Gemini CLI 指南文件中。
- neuDrive 不自动修改 `GEMINI.md` 或 Gemini CLI 配置。
...
- hooks、MCP、plugin 配置会出现在转换报告里，由用户手动处理。它们会作为文件复制到转换副本中，但 neuDrive 不会自动注册、安装或启用。
- neuDrive 不自动安装 MCP server，不自动启用 plugin，不自动修改 hook。

## 安全规则

- 自动写入只允许写 Agent 对应的 neuDrive 管理目录。
- `.neudrive-managed.json` 是更新和清理的判断依据。
- 同名目录如果不是 neuDrive 管理，预览会显示冲突，应用时不会写入。
- 清理只处理已取消分配且带 neuDrive 标记的目录，不删除用户手工创建的 Skill。
...
- `internal/api/sqlite_shared_test.go`
  - 导入复杂 Skill zip 后生成 `manifest.neudrive.json`，识别 scripts、requirements、package、assets、二进制、`external/claude-tools`、`external/claude-plugins`、环境变量提示。
  - Cursor 和 Gemini CLI 的分配会出现在本地同步预览中，但只有 `export` 项，不产生本地写入计划；导出包保留完整 Skill 文件夹。
  - Claude Code / Codex 本地应用只写目标管理目录，并用 `.neudrive-managed.json` 标记；同名非管理目录会返回冲突，不覆盖。
