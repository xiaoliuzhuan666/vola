...
- README、部署可靠性文档、GitHub Backup 文档都明确写到：
  ...
  - WebDAV / S3-compatible 上传的是 neuDrive 导出 zip。
...
- `GOCACHE=/private/tmp/neudrive-go-cache go test ./...`
  - 首次失败，原文为：
...
- `make build`：通过，已刷新嵌入式前端产物并生成 `bin/neudrive`、`bin/neu`。
- `docker build -t neudrive:launch-acceptance-20260518 .`：通过，镜像 ID 为 `0d78e3f39811`。
...
- 首页 HTML：通过，`title` 为 `Personal data hub for AI agents — neuDrive`。
- 页面渲染：
  - `/`
  - `/team`
  - `/sync-backup`
...
- `POST /api/backup/restore/preview`
  - 能识别 neuDrive export zip。
  - 返回分类：`identity`、`scratch`、`skills`、`unknown`。
- `POST /api/backup/restore/apply?mode=skip`
  - 成功执行。
  - 本次结果：`applied: 2`、`skipped: 27`、`overwritten: 0`。
- WebDAV 本地接收器
  - 收到 `PUT /webdav/neudrive-export-20260518-115920Z.zip`
  - 带 Basic auth
- S3-compatible 本地接收器
  - 收到 path-style `PUT /bucket/acceptance/neudrive-export-20260518-115948Z.zip`
  - 带 AWS4 签名
- `GET /api/backup/runs`
...
- Team Library 第一阶段完整主流程：
  ...
  - Agent `scope=team`
  - 团队资料进入真实备份
- 复杂 Skill 从导入到 `manifest.neudrive.json`、Agent 分配、本地同步、转换报告的本次实际操作复验。
